...
for i in range(sta.shape[0]):
    ...
    for j in range(len(cof_li)):
        if sta[i, 0] == cof_li[j][0] and sta[i, 1] == cof_li[j][1] and sta[i, 3] == cof_li[j][2]  and sta[i, 4] == cof_li[j][3] and sta[i, 5] == cof_li[j][4] and sta[i, 6] == cof_li[j][5]:
            index = j
    if i == 0 or index == -1:
        cof_li.append(sta[i, [0, 1, 3,  4, 5, 6]])
        sta_li.append([sta[i, 7:10]])
    else:
        sta_li[index].append(sta[i, 7:10])
