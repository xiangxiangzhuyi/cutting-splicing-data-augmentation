...
def read_tr_img(img, mask, bat_index, w_num, h_num, d_num):
    w_p = img.shape[1]/w_num
    h_p = img.shape[2]/h_num
    d_p = img.shape[3]/d_num
    # splice images
    img_li = []
        ...
        for d in range(d_num):
            ...
            for h in range(h_num):
                ...
                for w in range(w_num):
                    ...
                    d1 = np.ceil(d * d_p).astype(np.uint16)
                    d2 = np.ceil((d + 1) * d_p).astype(np.uint16)
                    im_p = all_im_ma[ind][w1:w2, h1:h2, d1:d2, :]
                    w_li.append(im_p)
                w_img = np.concatenate(w_li, 0)
                h_li.append(w_img)
            h_img= np.concatenate(h_li, 1)
            d_li.append(h_img)
        d_img = np.concatenate(d_li, 2)
        img_li.append(d_img)
...
def save_img(img, mask, fo_na):
    ...
    img, mask = img[:,:,:, 0], np.argmax(mask, -1).astype(np.uint8)
    for i in range(img.shape[0]):
        im = img[i]
        ma = mask[i]
        # save as png images
        cv2.imwrite(folder_name + 'img/' + str(i) + '.png', im)
        cv2.imwrite(folder_name + 'mask/' + str(i) + '.png', ma*80)
        # save as array
...
def ch_glob(tr_loss):
    ...
    # check
    if len(gl_loss) >= 2:
        if tr_loss.shape[0] > 5000 and gl_loss[-1] >= gl_loss[-2]:
            return True
        else:
            return False
