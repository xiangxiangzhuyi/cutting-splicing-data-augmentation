...
class UNET:
    def __init__(self, i_sh, o_sh, lean_rate, sa_str, train_flag, GPU_str = 'no'):
        # clean graph
        tf.reset_default_graph()
        # data parameters
        self.image_w, self.image_h, self.image_d, self.image_c = i_sh
        self.mask_w, self.mask_h, self.mask_d, self.class_num = o_sh
        self.learning_rate = lean_rate
        
        # placeholder
        self.input_data = tf.placeholder(tf.float32, [None, self.image_w, self.image_h, self.image_d, self.image_c])
        self.output_mask = tf.placeholder(tf.float32, [None, self.mask_w, self.mask_h, self.mask_d, self.class_num])
    ...
    def build_framework(self):
        # Unet
        self.output = unet.Unet(name="UNet", in_data = self.input_data, channel = self.image_c, 
                                cal_num = self.class_num, is_train = self.is_train, reuse = False)
        # loss
